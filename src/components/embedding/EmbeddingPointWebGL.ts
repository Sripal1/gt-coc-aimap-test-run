...
export function highlightPoint(
  ...
  if (point === undefined) {
    ...
    pointMouseleaveTimer = window.setTimeout(() => {
      ...
      this.topSvg.selectAll('g.point circle.point-border')
        .style('stroke', '#ffffff')
        .style('stroke-width', `${1 / this.curZoomTransform.k}px`);

      this.topSvg.selectAll('g.point').classed('point-hovered', false);
  ...
  // console.log("Current hoverd point " + this.hoverPoint.name)
  if (point === undefined) {
    currHoveredPoint.set(null);
  } else {
...
<div style="color: black;">${point.name}</div>`;
  ...
  // Find and update the point's visual state
  const pointElement = this.topSvg.selectAll('g.point')
    .filter((d: any) => d && d.id === point.id);

  if (!pointElement.empty()) {
    // Mark point as hovered using CSS class
    pointElement.classed('point-hovered', true);

    const borderElement = pointElement.select('circle.point-border');


    if (animated) {
      // Reset all other borders
      this.topSvg.selectAll('g.point')
        .filter((d: any) => d && d.id !== point.id)
        .classed('point-hovered', false);
